...
cancerType <- "SARC"
dir.create(path = paste0("MLSeqSurv_voomFeatures/", cancerType))
dir.create(path = paste0("MLSeqSurv_voomFeatures/", cancerType, "/test"))
dir.create(path = paste0("MLSeqSurv_voomFeatures/", cancerType, "/train"))
dir.create(path = paste0("MLSeqSurv_voomResults/", cancerType))
dir.create(path = paste0("MLSeqSurv_voomResults_time/", cancerType))
dir.create(path = paste0("MLSeqSurv_voomSelectedFeatures/", cancerType))
...
for (index in 1:30) {
  ...
  train_data <- read.csv(paste("Results_CompSurv/", cancerType,"_ProteinCoding/trainset/trainset", index, ".csv", sep = ""), header = TRUE, sep=",")
  test_data <- read.csv(paste("Results_CompSurv/", cancerType,"_ProteinCoding/testset/testset", index, ".csv", sep = ""), header = TRUE, sep=",")
  ...
  #### voom transformation ####
  train_E <- donusum$E
  train_W <- donusum$targets$sample.weights

  test_E <- donusum$TestExp
  ...
  ### Write final version of train and test datasets ###
  fwrite(tvsd_train2, file = paste("MLSeqSurv_voomFeatures/",cancerType, "/train/train_", index, ".csv", sep = ""))
  fwrite(tvsd_test2, file = paste("MLSeqSurv_voomFeatures/",cancerType, "/test/test_", index, ".csv", sep = ""))
